...
CREATE TABLE public.whishlist (
    ...
    id_livro integer,
    visibilidade character varying(20) DEFAULT 'publico'::character varying NOT NULL,
    CONSTRAINT whishlist_visibilidade_check CHECK (((visibilidade)::text = ANY ((ARRAY['publico'::character varying, 'privado'::character varying])::text[])))
);
...
COPY public.conta (id_user, foto_perfil_url, bio, visibilidade) FROM stdin;
3	foto_do_gomez	Zagueiro Paraguaio, dono da sele‡Æo paraguaia e maior capitÆo da hist¢ria do Palmeiras	publico
\.
...
COPY public.whishlist (id, nome_lista, id_user, id_livro, visibilidade) FROM stdin;
\.
